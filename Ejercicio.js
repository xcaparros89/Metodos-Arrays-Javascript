...
let ArrayFinalNote = (array) => {
return array.map(obj=>{
  let finalNote = ((obj.firstProject+obj.secondProject)/2*0.4)+(obj.finalExam*0.6);
  return {
    name: obj.name,
    finalNote:finalNote
  }
...
console.log(ArrayFinalNote(students))
...
//Iteration 4
  ...
  let newProduct = obj =>{
    let sum = obj.reviews.reduce((acc, num)=>
       acc + num.rate
    ,0)
...
//ES6 =>
let canDrink = people => people.filter(person=>person.age>20);
console.log(canDrink(people))
...
//ES6 =>
let filterPools = array => array.filter(house=>house.pool)
filterPools(places)
...
let filterNumbers = array => array.filter(num=>num>42 && num%2 ===0)
